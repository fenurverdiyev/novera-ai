...
import { VideoIcon, ShoppingCartIcon, MapPinIcon, MapIcon, MusicIcon } from './LiveIcons';
...
export const LiveMessageDisplay: React.FC<MessageDisplayProps> = ({ conversation, sources, searchResults, status }) => {
  ...
  const handleCardClick = (item: SearchResultItem) => {
    ...
    if (item.type === 'video' || (item.type === 'music' && embedUrl)) {
      if (embedUrl) {
        ...
        try { window.open(item.source, '_blank', 'noopener,noreferrer'); } catch {}
      }
  ...
  const lastUser = [...conversation].reverse().find(t => t.author === 'user') || null;
  const intentText = `${lastUser?.text || ''} ${lastTurn?.text || ''}`.toLowerCase();
  const wantsProducts = /(məhsul|qiymət|satın|almaq|haradan ala|mağaza|magaza|price|buy|purchase|amazon|aliexpress)/i.test(intentText);

  const visibleResults = (searchResults || []).filter(item => item.type !== 'product' || wantsProducts);
  const hasSources = !!(sources && sources.length > 0);
  const hasSearchResults = visibleResults.length > 0;

  // Typing effect for model lastTurn
  useEffect(() => {
    if (!lastTurn) { setDisplayText(''); setIsStreaming(false); return; }
    if (lastTurn.author !== 'model') { setDisplayText(lastTurn.text || ''); setIsStreaming(false); return; }
    const t = lastTurn.text || '';
    if (!t) { setDisplayText(''); setIsStreaming(status === 'processing' || status === 'speaking'); return; }
    let i = 0;
    setDisplayText('');
    setIsStreaming(true);
    const step = Math.max(1, Math.round(t.length / 480));
  ...
  }, [lastTurn?.text, lastTurn?.author, status]);
    ...
    <>
      <div className="relative w-full min-h-[8rem] px-1 py-4 bg-black/20 backdrop-blur-sm border border-white/10 rounded-lg flex flex-col items-center justify-center text-center shadow-2xl shadow-black/50 transition-all duration-300">
        ...
        <div className="flex-grow flex items-center justify-center px-5">
          <p className={`text-xl sm:text-2xl text-gray-100 transition-opacity duration-300 animate-[fadeIn_0.5s_ease-in-out] ${isStreaming ? 'animate-[breath_2.4s_ease-in-out_infinite]' : ''}`}>
            {lastTurn ? (
              <span key={conversation.length}>
                <span className={`font-bold capitalize ${lastTurn.author === 'model' ? 'text-cyan-300' : lastTurn.author === 'user' ? 'text-purple-300' : 'text-gray-400'}`}>
                  {lastTurn.author === 'model' ? 'NovEra' : lastTurn.author === 'user' ? 'Siz' : 'Sistem'}: 
                </span>
                {displayText}
                {isStreaming && <span className="inline-block w-[6px] h-5 align-[-2px] bg-white/85 rounded-[1px] ml-1 animate-[blink_1.1s_steps(2,_start)_infinite]"></span>}
              </span>
            ) : (
        ...
        <style>{`
          ...
          @keyframes breath {0%{opacity:.92}50%{opacity:.68}100%{opacity:.92}}

...
import { NewsIcon, WeatherIcon, TranslateIcon, SettingsIcon, UserIcon, PlusIcon, SparklesIcon, BookmarkIcon, CloseIcon, GlobeIcon } from './Icons';
...
export const Sidebar: React.FC<SidebarProps> = ({ activeView, setActiveView, themeColor, onNewChat, onOpenSession }) => {
  ...
  const clearAllSessions = () => {
    const sure = window.confirm('Bütün keçmiş söhbətləri silmək istədiyinizə əminsiniz?');
    if (!sure) return;
    try {
    ...
    <nav className="w-60 bg-bg-slate/90 backdrop-blur-sm flex flex-col py-4 border-r border-white/10">
          ...
          <div className="flex items-center justify-between px-2">
            ...
            {sessions.length > 0 && (
              <button
                ...
                className="text-[10px] px-2 py-1 rounded-md bg-white/5 hover:bg-white/10 text-white/70 border border-white/10"
                title="Hamısını sil"
              >
                Hamısını sil
              </button>
      ...
      <div className="px-3 pt-3 border-t border-white/10">
        ...
        <div className="mt-2 grid grid-cols-2 gap-2">
          ...
          {accountEmail ? (
            <button
              onClick={() => { try { localStorage.removeItem('nov-era-auth'); } catch {}; setAccountEmail(null); }}
              className="px-3 py-2 rounded-lg bg-white/5 hover:bg-white/10 text-white/80 text-sm"
            >
              Çıxış
            </button>

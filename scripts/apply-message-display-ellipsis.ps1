...
const formatText = (text: string) => {
  ...
  return text.split('\n').map((line, lineIndex) => {
    if (line.trim() === '') {
      return <div key={lineIndex} className="h-4" />; // spacing for empty lines
    }
      ...
      <div className="flex items-center gap-3">
        {steps.map((s, idx) => {
          ...
          const chipClass = (active
            ? 'px-3 py-1 rounded-full text-xs border bg-accent/20 text-accent border-accent/30'
            : 'px-3 py-1 rounded-full text-xs border bg-white/5 text-white/70 border-white/10');
          const dotClass = (active ? 'inline-block w-2 h-2 rounded-full mr-2 bg-accent animate-pulse' : 'inline-block w-2 h-2 rounded-full mr-2 bg-white/30');
          const connectorClass = (active ? 'w-6 h-px bg-accent/50' : 'w-6 h-px bg-white/10');
          return (
            <div key={s.id} className="flex items-center gap-2">
              <div className={chipClass}>
                <span className={dotClass}></span>
                {s.id !== 3 && <span className="align-middle">{s.label}</span>}
                {s.id === 3 && (
                  <span className="inline-flex items-center ml-1 align-middle">
                    <span className="w-1.5 h-1.5 rounded-full bg-black/80 opacity-70 animate-pulse"></span>
                    <span className="w-1.5 h-1.5 rounded-full bg-black/80 opacity-70 animate-pulse ml-0.5" style={{ animationDelay: '150ms' }}></span>
                    <span className="w-1.5 h-1.5 rounded-full bg-black/80 opacity-70 animate-pulse ml-0.5" style={{ animationDelay: '300ms' }}></span>
                  </span>
                )}
              </div>
              {idx < steps.length - 1 && <div className={connectorClass}></div>}
            </div>
          );
        })}
      </div>
...
Set-Content -LiteralPath 'd:\NovEra\NovEra\components\MessageDisplay.tsx' -Value $content -Encoding UTF8
Write-Host 'MessageDisplay.tsx updated (ellipsis for step 3).'
